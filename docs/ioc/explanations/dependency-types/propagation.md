...
For example,
if we were to package {ref}`pkg-support.StreamDevice`,
we would need to specify {ref}`pkg-support.asyn` as a propagated dependency:
...
Another example is how using the {ref}`pkg-support.autosave` support module
might lead you to manually load the {ref}`pkg-support.busy` support module.
