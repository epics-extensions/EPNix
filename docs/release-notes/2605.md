...
- {pkg}`epnix.archiver-appliance` was upgraded to `2.4.0+`
  which introduced Parquet support
  and now requires Tomcat 10.
  This Tomcat 10 requirement unfortunately makes it incompatible
  with the {option}`services.dbwr` and {option}`services.pvws` modules,
  which require Tomcat 9.
...
- EPNix now enables [structured attributes](https://nixos.org/manual/nixpkgs/stable/#var-stdenv-__structuredAttrs)
  by default on `mkEpicsPackage` packages.
...
- A new guide for {doc}`../ioc/user-guides/deploying` on non-NixOS systems.

- New guides for using flake inputs:
  {doc}`../ioc/user-guides/specifying-flake-inputs`
  and {doc}`../ioc/user-guides/dependency-updates`.

- We now serve the [EPNix documentation on ReadTheDocs](https://epnix.readthedocs.io/),
  with the hope of migrating from GitHub pages in the future.
  ReadTheDocs has better link preview,
  better multi-version management,
  should have less issue with SEO,
  and is open source.

- A new {doc}`../contributing/explanations/no-llms-policy`
  that forbids using LLM for contributing to EPNix.
