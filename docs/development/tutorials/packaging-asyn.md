...
Sometimes,
you might want to depend on an EPICS support module
...
Before suggesting changes to EPNix,
you need to fork it.
...
:::{tip}
If you want quick access to your EPNix repository
...
no matter your current working directory.
:::

## Create a feature branch

Before creating commits to EPNix,
make sure to create an appropriately named branch:
...
see the {doc}`../../ioc/explanations/dependency-types` explanation.

For EPICS support module packages,
the key points are:

% NOTE: When updating this section,
% remember to update the linked explanation

- Add EPICS support modules to `propagatedBuildInputs`.
- Add system libraries to both `nativeBuildInputs` and `buildInputs`.
- Add tools required only during the build process to `nativeBuildInputs`.
- Some exceptions,
  such as `pkg-config`
  or other tools that generate native code
  or search for system libraries,
  should be added to both `depsBuildBuild` and `nativeBuildInputs`.
- More attributes are available for more complex cases.
