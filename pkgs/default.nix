...
recurseIntoAttrs {
  ...
  epnix = scope prev (
...

    {
      ...
      phoebus-ecosystem = scope self (self: {
        virtualbox-image = self.callPackage ./phoebus-ecosystem/virtualbox-image.nix { };
        qemu-vm = self.callPackage ./phoebus-ecosystem/qemu-vm.nix { };
      });
